...
class RegressionSquaredErrorObjectiveExtractorTest {

    @Nested
    class WithMinMaxNormalizer {

        private final RegressionSquaredErrorObjectiveExtractor extractor =
                new RegressionSquaredErrorObjectiveExtractor(new MinMaxNormalizer(0.0, 10.0));
        ...
        void maximum_raw_value_normalizes_to_1() {
            assertEquals(1.0, extractor.extract(new float[][]{{10f}}).at(0).value(), 1e-9);
        }

        @Test
        void midpoint_raw_value_normalizes_to_0_point_5() {
            assertEquals(0.5, extractor.extract(new float[][]{{5f}}).at(0).value(), 1e-9);
        }
    }
    ...
    class ExtractionMechanics {
        ...
        private final RegressionSquaredErrorObjectiveExtractor extractor =
                new RegressionSquaredErrorObjectiveExtractor(v -> (double) v);

        @Test
        void normalizer_receives_first_element_of_each_row() {
            var result = extractor.extract(new float[][]{{3f}, {7f}});
